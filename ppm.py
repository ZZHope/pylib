...
Python module for reading Yprofile-01-xxxx.bobaaa files


Simple session for working with ppm.py, here I assume user's working directory 
contains some YProfile files.

Assumptions on the nature of Yprofile files:
labeled as YProfile-01-xxxx.bobaaa and the xxxx is the NDump that is located within each file.
There can be a maximum of 9999 files in each directory.
The first 10 lines of each file  are the only place where header data is uniquely located.
Header data is separated by five spaces and line breaks.
An attribute cant be in the form of a number, ie if the user is able to 'float(attribute)' (in python) 
	without an error attribute will not be returned.
A row of column Names or Cycle Names is preceded and followed by a blank line.
A block of data values are preceded and followed by a blank line except the last
	data block in the file, where it is not followed by a blank line.
In the YProfile file, if a line of attributes contains the attribute Ndump, then
	that line of attributes and any other following lines of attributes in the file
	are cycle attributes.
Header Attribute Assumptions:
Header attributes are separated by lines and instances of four spaces (    )
Header attribute come in one ofbelow here are things that do stuff with the data 6 types.
The first type is the first attribute in the file. It is formatted in such a way such that
	the name of the attribute is separated from its associated data with an equals sign
	ex. Stellar Conv. Luminosity =  1.61400E-02 x 10^43 ergs,
The second type is when an attribute contains the sub string 'grid;'. It is formatted in such a way such that
	there are 3 numerical values separated by 'x' which are then followed by the string ' grid;'
	ex. 384 x 384 x 384 grid;
The third type is were name of the attribute is separated from its associated data with an equals sign.
	Also that data may be followed by a unit of measurement. 
	ex. Thickness (Mm) of heating shell =  1.00000E+00
The fourth type is when an attribute contains a colon. The String before the colon is the title of 
	the attribute.  After the colon there is a list of n sub attributes, 
	each with a sub title, and separated by its value with an equals sign.  Aslo each sub attribute is separated by a comma
	ex. At base of the convection zone:  R =  9.50000E+00,  g =  4.95450E-01,  rho =  1.17400E+01,  p =  1.69600E+01
The fifth is when an attribute starts with 'and'. after the and, the next word 
	after has to be the same as one word contained in the previous attribute.
	ex. and of transition from convection to stability =  5.00000E-01  at R =  3.00000E+01 Mm.
The sixth is when there is a string or attribute title followed by two spaces followed by one value 
	followed by two spaces followed by an 'and' which is then followed by a second Value
	ex. Gravity turns on between radii   6.00000E+00  and   7.00000E+00  Mm.
...
class YProfile(DataPlot):
	...
	def getHeaderData(self,attri):
		...
		Input: attri, the name of the attribute.
		To see all possable options in this instance type instance.getHattrs()
		"""
		isHead = False
	...
	def getCycleData(self, attri,FName=None,numType='ndump',Single=False,resolution='H'):
		"""
		Returns a list (Datalist) of values for the given attribute or a single attribute
		in the file FName.
		Input:
	...
	def getColData(self, attri, FName, numType='ndump' ,resolution='H', cycle=False):
		"""
	...
	def findFile(self,FName, numType='FILE'):
		...
		if numType is 'NDump' function will look at the cycle with that nDump
		if numType is 'T' or 'time' function will find the cycle with the closest
		time stamp 
		Fname the name of the file we are looking or Ndump or time
		"""
	...
	def _formatHeader(self):
    		"""
    		Private function that takes in a set of header attributes and then 
    		Formats them into a dictionary
    		...
    		The output in the dictionary looks like {'Stellar Conv. Luminosity':The associated data}
    			If an element contains the string 'grid;' it is the grid size header
    		and the first, second and third are the x, y and z grid sizes respectively
    		The output in the dictionary looks like {'gridX':9,'gridY':9,'gridZ':9}
    			If an element is size two the first item is the header name and the second will 
    		be its associated value.
    		The output in the dictionary looks like {'First Item':value}
    			If an element contains a colon, The string preceding the colon is one part of
    		the header name. The string after the colon will be a list of associations
    		in the form of the name followed by an equals sign followed by its value.
...
# below are some utilities that the user typically never calls directly
	def readTop(self,atri,filename,stddir='./'):
		"""
		Private routine that Finds and returns the associated value for attribute in 
		the header section of the file.
		Input:  atri, what we are looking for
			filename where we are looking
			StdDir the directory where we are looking, Defaults to the working
			Directory
	...
	def _readFile(self,filename,stddir='./'):
		   """ private routine that is not directly called by the user.
			...
			Assumprions: *An attribute cant be in the form of a num, if 
